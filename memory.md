...
- **Техно-декор західної стіни** (план узгоджено з користувачем): 3 плакати
  біля дверей ПРИБРАНО; замість них однолінійний робот-декаль
  (`ROBOT_LINE_DECAL`, на золотому перерізі прогону `WEST_NEON_SPAN`
  [−70..400], y=220) + анімована неонова LED-хвиля (рожевий→фіолет→
  бірюза→синій, «тече» через `texture.offset.x` — Pattern в
  infrastructure/insights.md). Нові текстури makeRobotLineArtTexture /
  makeNeonGradientTexture; `RoomBuild.updatables` → sceneAssembler.
  Постери східної стіни (між вікнами і біля фронту) збережені свідомо.
  139 тестів зелені; анімацію верифіковано порівнянням піксель-семплів
  двох кадрів.
...
- **2026-07-17 — Техно-декор західної стіни: неон + однолінійний робот
  (гілка feat/modified_design)** За референсом-фото (неон + line-art
  космонавт) і вибором користувача: робот замість космонавта, анімований
  перелив. Прибрано 3 плакати біля дверей. Домен: `WEST_NEON_SPAN`
  {from:−70,to:400} + `ROBOT_LINE_DECAL` (y=220 — золотий переріз прогону).
  Інфраструктура: neonStrip() — CatmullRom-крива з «провалом» під колесами
  робота, TubeGeometry + MeshBasicMaterial(toneMapped:false) з безшовним
  градієнтом, анімація texture.offset.x; robotLineDecal() — прозорий
  декаль-план у групі west (dollhouse ✓). RoomBuild.updatables (нове поле)
  → updatables сцени. 139 тестів (+4), снапшот декору записався без -u.
...
---
Останнє оновлення: 2026-07-17 (техно-декор західної стіни, feat/modified_design)
