...
/**
 * Техно-декор західної стіни (протилежної до вікон), редизайн 2026-07:
 * неонова LED-хвиля над моніторами західної колони + однолінійний робот.
 * Прогін неону — південніше дверей (двері закінчуються на y ≈ −103),
 * уздовж усієї колони учнівських столів. Єдине джерело правди для
 * інфраструктурної кривої неону та тестів композиції.
 */
export const WEST_NEON_SPAN = { from: -70, to: 400 };

/** Однолінійний робот (графітовий контур, за референсом): центр — на
 *  золотому перерізі прогону неону (−70 + 470/φ ≈ 220); неон «протікає»
 *  під його ногами. Фронт — на схід, у кімнату. */
export const ROBOT_LINE_DECAL: PlacedItem = {
  x: -34.4, y: 220, elevation: 150, angle: (3 * Math.PI) / 2, width: 100, depth: 1, height: 95,
};
